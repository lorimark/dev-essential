...
void Composer::setFrom(const std::vector<Contact>& from)
{
  fromEdit_->setAddressees(from);
}
...
std::vector<Contact> Composer::from() const
{
  return fromEdit_->addressees();
}
...
void Composer::addAttachment( const Attachment & attachment )
{
...
void Composer::createUi()
{
  ...
  edits_->elementAt(0, 0)->resize(WLength(1, LengthUnit::Percentage), WLength::Auto);
   ...
   */
  int row = 0;
  fromEdit_ = edits_->elementAt(row,1)->addWidget(cpp14::make_unique<AddresseeEdit>(tr("msg.from"), edits_->elementAt(row, 0)));
  row++;

  /*
   * To, Cc, Bcc
   */
  toEdit_ = edits_->elementAt(row,1)->addWidget(cpp14::make_unique<AddresseeEdit>(tr("msg.to"), edits_->elementAt(row, 0)));
  // add some space above To:
  edits_->elementAt(row, 1)->setMargin(5, Side::Top);
  row++;
  ccEdit_ = edits_->elementAt(row,1)->addWidget(cpp14::make_unique<AddresseeEdit>(tr("msg.cc"), edits_->elementAt(row, 0)));
  row++;
  bccEdit_ = edits_->elementAt(row,1)->addWidget(cpp14::make_unique<AddresseeEdit>(tr("msg.bcc"), edits_->elementAt(row, 0)));
  row++;
   ...
   * by pipe characters.
   */
  options_ = edits_->elementAt(row, 1)->addWidget(cpp14::make_unique<OptionList>());
  row++;
   ...
   * Subject
   */
  edits_->elementAt(row, 0)->addWidget(cpp14::make_unique<Label>(tr("msg.subject"), edits_->elementAt(row, 0)));
  subject_ = edits_->elementAt(row, 1)->addWidget(cpp14::make_unique<WLineEdit>());
   ...
   * Attachments
   */
  edits_->elementAt(row, 0)->addWidget(cpp14::make_unique<WImage>("resources/icons/paperclip.png"));
  edits_->elementAt(row, 0)->setContentAlignment(AlignmentFlag::Right | AlignmentFlag::Top);
  edits_->elementAt(row, 0)->setPadding(3);
  ...
  AttachmentEdit *attachmentEdit = edits_->elementAt(row, 1)->addWidget(cpp14::make_unique<AttachmentEdit>(this));
   ...
   */
  attachFile_ = edits_->elementAt(row, 1)->addWidget(cpp14::make_unique<Option>(tr("msg.attachfile")));
  attachOtherFile_ = edits_->elementAt(row, 1)->addWidget(cpp14::make_unique<Option>(tr("msg.attachanother")));
